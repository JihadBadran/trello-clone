...
export type KanbanState = BoardsSlice &
  ColumnsSlice &
  CardsSlice & {
    repos: {
...
const createKanbanSlice: StateCreator<KanbanState, [], []> = (
  set,
  get,
  api
) => {
...
export const makeKanbanStore = (deps: {
  publish: (action: Action) => void;
  tabId: string;
  dev?: boolean;
}) => {
  const store = create<KanbanStore>()(
    devtools(withKanbanActions(deps)(createKanbanSlice as any), {
      name: 'KanbanStore',
      enabled: deps.dev,
    })
  );
